...
| --- | --- | --- | --- |
| `2026-03-26` | `0.1.15` | `0.1.15` | 1. 回填分段评分表解析术语：支持“维度标题行 + 评分项行 + 续行标准”的版式识别。<br>2. 回填评分项继承语义：续行标准继承最近有效的维度与评分项归属。<br>3. 回填协作治理执行校验语义：执行前先读取 `docs/docs-index.md` 并完整重读 `docs/00-governance/`。<br>4. 回填 `project-docs` 同步语义：项目文件变更需按映射规则同步更新说明并在交付中声明结果。<br>5. 对齐 `REQ-v0.1.15-004/005` 已发布事实。 |
...
| --- | --- | --- | --- |
| `0.1.15` | `2026-03-26` | `0.1.15` | 回填分段评分表解析术语：评分表支持维度标题行/评分项行/续行标准版式识别，续行标准继承最近有效的维度与评分项归属；补充协作治理术语：执行前重读 `docs/docs-index.md` 与 `docs/00-governance/`，项目文件变更需同步更新 `project-docs` 并在交付中声明同步状态。 |
