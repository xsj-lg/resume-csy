...
| field | name_zh | allowed_values | desc_zh |
| --- | --- | --- | --- |
| `pdf_parse_source` | PDF 解析来源 | `parser_service_primary_with_legacy_fallback` | 默认使用本地 PDF 解析服务接口读取文本，服务失败时自动回退旧的本地 PDF 文本提取工具。 |
| `pdf_parse_config_source` | PDF 解析配置来源 | `config_file_first_env_override` | 运行参数优先来自独立配置文件，支持环境变量覆盖配置文件路径与单项参数。 |
| `pdf_parse_cache_policy` | PDF 解析缓存策略 | `candidate_files_text_and_payload_cache` | 解析文本、原始载荷与更新时间统一写入 `candidate_files`，供后续结构化抽取与自动评分复用。 |
| `pdf_parse_cache_reuse_policy` | PDF 解析缓存复用策略 | `db_first_then_parse` | 后续读取优先命中数据库中的已落库解析结果，仅在缓存缺失、无效或显式刷新时重新解析。 |
| `pdf_parse_failure_policy` | PDF 解析失败策略 | `service_failure_auto_fallback_and_cache` | 当解析服务不可达、超时、异常或结果为空时，自动回退旧工具，并把回退结果继续写入数据库缓存。 |
...
| `profile_replace_strategy` | 字段替换策略 | `non_empty_override_keep_existing` | 结构化抽取结果仅在字段存在有效值时覆盖当前通用信息；空值或无效值保留原有结果。 |
| `profile_update_trigger` | 抽取刷新触发 | `manual_refresh_then_sync` | 触发“更新抽取”后，同步刷新通用信息展示与保存态数据。 |
| `profile_name_sync_policy` | 候选人名称同步策略 | `structured_name_sync_to_candidate_primary_name` | 当结构化抽取得到有效姓名时，同步更新候选人主名称字段，保持列表与详情名称来源一致。 |
