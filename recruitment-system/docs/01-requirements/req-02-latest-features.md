...
| `2026-03-17` | `0.1.10` | `0.1.10` | 1. 发布用户管理编辑能力补齐：用户编辑改为弹窗表单，支持显示名、状态、角色、部门范围一次性编辑。<br>2. 发布角色与部门范围联动校验：角色为部门负责人时部门范围必填，其他角色不生效。<br>3. 发布用户状态与角色入参校验加固：前后端统一校验非法状态与角色编码并返回明确错误。 |
...
| 简历结构化抽取与通用信息融合展示 | `available` | `web/index.html`, `web/app.js`, `app/backend/services/recruitment_service.py` | 抽取结果按映射写入通用信息口径；抽取失败时保留既有通用信息并提示失败状态。 |
