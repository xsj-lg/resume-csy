...
| --- | --- | --- | --- |
| `2026-03-26` | `0.1.15` | `0.1.15` | 1. 补录已发布需求 `REQ-v0.1.15-004`：上传评分表解析支持“维度标题行 + 评分项行 + 续行标准”的分段结构。<br>2. 补录已发布需求 `REQ-v0.1.15-005`：协作治理新增执行前强制重读 `docs/00-governance/` 与 `project-docs` 同步要求。<br>3. 同步完成 `REQ-v0.1.15-004/005` 的 `req-01/req-02` 发布后回填。 |
...
| --- | --- | --- | --- | --- |
| `v0.1.15` | `RELEASED` | `2026-03-26` | 补录发布评分表解析增强：评分表上传解析支持“维度标题行 + 评分项行 + 续行标准”的分段结构，评分预览与岗位评分项快照可正确保留维度与评分项归属；补录发布协作治理增强：Agent 执行前需先读取 `docs/docs-index.md` 并完整重读 `docs/00-governance/`，涉及代码或文件变更时必须同步更新对应 `project-docs` 描述并在交付中说明。 | `app/backend/services/score_table_service.py`, `app/backend/services/job_service.py`, `docs/00-governance/gov-03-agent-collaboration.md`, `project-docs/development/development-and-integration-guidelines.md`, `docs/01-requirements/*`, `docs/02-roadmap/*` |
